...
class TestSuite6(BaseTestSuite):
    ...
    def test_compute_metrics(self, target):
        ...
        try:
            # 测试用例1：给定的混淆矩阵
            cm1 = np.array([[1027, 33], [52, 828]])

            # 手工计算预期结果
            # 统一采用 [[TN, FP], [FN, TP]]：TN=1027, FP=33, FN=52, TP=828
            # accuracy = (1027 + 828) / (1027 + 33 + 52 + 828) = 1855/1940 ≈ 0.9562
            # recall = 828 / (828 + 52) = 828/880 ≈ 0.9409
            # precision = 828 / (828 + 33) = 828/861 ≈ 0.9617
            # f1_score ≈ 0.9512
            ...
            expected_metrics1 = {
                ...
                "recall": 828/880,
                "precision": 828/861,
                "f1_score": 2 * (828/861) * (828/880) / ((828/861) + (828/880))
            }
            ...
            self.test_results[test_name] = 1
            print(colored(f"习题9通过 {test_name} 测试。{sum(self.test_results.values())}/{len(self.test_results)}", "green"))
